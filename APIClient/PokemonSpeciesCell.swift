...
class PokemonSpeciesCell: UITableViewCell {
    ...
    func configureForEntry(entry: Pokedex.Entry, pokemonSpecies: APIResource<PokeAPI, PokemonSpecies>, sprite: APIResource<PokeAPI, Pokemon>?) {
        ...
        if let sprite = sprite {
            ...
            case .loaded(let sprite):
                ...
                spriteImageview.hidden = false
                // Use `AlamofireImage` to load the image from the URL and display it in the image view
                spriteImageview.af_setImageWithURL(sprite.sprites.frontDefault.url)
            default:
                spriteImageview.hidden = true
            }
        } else {
            spriteImageview.hidden = true
        }
